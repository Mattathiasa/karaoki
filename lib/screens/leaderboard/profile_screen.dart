    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(children: [
          ...
          Expanded(
            child: ListView(padding: const EdgeInsets.symmetric(horizontal: KSpacing.mobilePaddingH), children: [
              const SizedBox(height: 16),
              Center(child: Column(children: [
                const KAvatar(initial: 'M', size: 74),
                const SizedBox(height: 12),
                const Text('Matt', style: TextStyle(fontFamily: 'BricolageGrotesque', fontWeight: FontWeight.w700, fontSize: 23, color: KColors.bone)),
              ...
              ...List.generate(5, (i) {
                ...
                return Padding(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      Container(width: 46, height: 46, decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), color: KColors.ink700), alignment: Alignment.center, child: const Icon(Icons.music_note, color: KColors.bone28, size: 18)),
                      const SizedBox(width: 12),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(songs[i], style: KTypography.uiRowTitle), Text(artists[i], style: KTypography.monoLabel.copyWith(fontSize: 10))])),
                      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(scores[i], style: TextStyle(fontFamily: 'BricolageGrotesque', fontWeight: FontWeight.w700, fontSize: 19, color: colors[i])), Text(ranks[i], style: KTypography.monoLabel.copyWith(fontSize: 9, color: colors[i]))]),
                    ]),
                  ),
                );
              }),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 4),
        Text(value, style: TextStyle(fontFamily: 'BricolageGrotesque', fontWeight: FontWeight.w700, fontSize: 18, color: highlighted ? KColors.lime : KColors.bone)),
      ]),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Container(
        ...
        decoration: BoxDecoration(color: selected ? KColors.lime : Colors.transparent, borderRadius: BorderRadius.circular(999)),
        alignment: Alignment.center,
        child: Text(label, style: KTypography.uiButton.copyWith(fontSize: 13, color: selected ? KColors.onAccent : KColors.bone55)),
      ),
    );
  }
}
